...
class VectorStore:
    ...
    def load_document(self):
        def check_md5_hex(md5:str):
            md5_hex_path = get_abs_path(chroma_conf["md5_hex_store"])
            if not os.path.exists(md5_hex_path):
                open(md5_hex_path,"w",encoding="utf-8").close()
                return False

            with open(md5_hex_path,"r",encoding="utf-8") as f:
